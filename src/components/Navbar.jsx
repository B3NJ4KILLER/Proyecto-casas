...
const Navbar = () => {
  ...
  // 1. DEFINIMOS DÓNDE QUEREMOS QUE SEA TRANSPARENTE
  const rutasTransparentes = ["/", "/comprar", "/arrendar"];

  // Verificamos si la ruta actual está en la lista de transparentes
  const esTransparente = rutasTransparentes.includes(location.pathname);

  // 2. CLASE DINÁMICA
  // Si es transparente -> Absolute (flota sobre la foto) + Gradiente
  // Si NO es transparente (ej: Detalle) -> Relative (empuja el contenido) + Fondo Gris Oscuro
  const navClass = esTransparente
    ? "absolute backdrop-blur-xs top-0 left-0 w-full z-50 bg-transparent h-24 flex items-center border-b border-b-white"
    : "relative w-full z-50 bg-gray-900 shadow-md";
  ...
  return (
    <nav className={navClass}>
      ...
      {/* --- MENÚ MÓVIL DESPLEGABLE --- */}
      <div
        className={`md:hidden absolute top-0 left-0 w-full bg-gray-950/95 backdrop-blur-sm shadow-xl transition-all duration-300 ease-in-out ${isOpen ? "opacity-100 visible pt-28 pb-10" : "opacity-0 invisible h-0 py-0 overflow-hidden"}`}
      >
        <div className="flex flex-col items-center space-y-6 text-white text-lg font-medium">
          <NavLink
            to="/"
